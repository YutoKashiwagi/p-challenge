...
リクエストがユニークかどうかの判断材料は通常URLとHTTPメソッドのみだが、Varyを利用して指定したHTTPヘッダーを判断材料に加えることができる

例: あるエンドポイントへのGETリクエストで多言語対応が必要な場合
...
- Request URL: https://zenn.dev/taichifukumoto/articles/how-to-use-volta

  - ページのHTMLがキャッシュされている

一回目のリクエスト時は200レスポンスと共にetagの値が返却される

二回目以降は前回受け取ったetagをIf-None-Matchヘッダーに含めてリクエストを送信し、304レスポンスが返却されている

### ProgateのLP

https://prog-8.com/

- https://prog-8.com/api/landing?locale=ja

LP用のコンテンツを取得するAPIにはキャッシュが利用されているっぽい

- 上記と同じ、検証モデルのキャッシュが利用されている
- 多言語対応をしているっぽいが、Varyヘッダーにはオリジンが指定されており、クエリパラメータで言語ごとのキャッシュが制御されている


# キャッシュを使うべきでない状況

- リアルタイムで正確な情報が求められる状況
- コンテンツが不規則に頻繁に書き換えられるページ

# クイズ

Cache-Contorol, Expiresなど、複数のヘッダーでキャッシュの設定がされている場合の優先順位を教えてください
